...
fluentqml_core_path = Path(__file__).resolve().parent  # FluentQML/core 目录

BASE_DIR = Path.cwd().resolve()
PATH = BASE_DIR / "config"
FLUENTQML_PATH = resource_path(
    fluentqml_core_path.parent.parent
)  # 使用 resource_path 处理路径，等同 ../../
FLUENTQML_QML_IMPORT_PATH = "qrc:/"
FLUENTQML_RESOURCE_PREFIX = ":/FluentQML"
...
class ConfigManager:
    def __init__(self, path, filename):
        ...
        :param filename: json config file name (eg: fluentqml.json)
        """
        self.path = Path(path)
...
FluentQMLConfig = ConfigManager(path=PATH, filename="fluentqml.json")
FluentQMLConfig.load_config(DEFAULT_CONFIG)  # 加载配置
